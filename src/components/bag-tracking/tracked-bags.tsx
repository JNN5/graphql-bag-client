...
const TrackedBags = () => {
  ...
  const fetchBags = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch tracked bags",
      );
    } finally {
      setIsLoading(false);
    }
                  ...
                  <div className="space-y-3">
                    <div className="space-y-2">
                      ...
                      <div className="text-sm">
                        Origin: {bag.origin} → Destination: {bag.destination}
                      </div>
                      <div
                      ...
                      {bag.vehicle_number && (
                        <div className="text-sm">
                          Vehicle: {bag.vehicle_number}
                        </div>
                      )}
                      {bag.location && (
                        <div className="text-sm">Location: {bag.location}</div>
                      )}
                    </div>
                    <div className="grid place-items-center">
                      {!isNaN(Number(bag.bag_tag_no)) &&
                        Number.isInteger(Number(bag.bag_tag_no)) && (
                          <Barcode text={bag.bag_tag_no} />
                        )}
                    </div>
                    <div className="grid grid-cols-3 gap-1">
                      {bag.bag_images?.map((image) => (
                        <Dialog
                          key={image.url}
                          open={selectedImage?.url === image.url}
                          onOpenChange={(open) => {
